...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
      ...
      // [JS] boardIds를 찾기 위한 함수 설정
      const findBoardIds = (obj: any): any => {
        if (!obj || typeof obj !== 'object') return null;
        if (obj.COMPLAINT && obj.NOTICE && obj.POLL) return obj;
        if (obj.boardIds) return obj.boardIds;
        for (const key in obj) {
          const result = findBoardIds(obj[key]);
          if (result) return result;
        }
        return null;
      };

      const extractedBoardIds = findBoardIds(userData) || {
        COMPLAINT: userData.complaintBoardId || '',
        NOTICE: userData.noticeBoardId || '',
        POLL: userData.pollBoardId || '',
      };
      ...
      setUser({
        ...
        // boardIds: userData.boardIds, 기존 BoardId 방식
        boardIds: extractedBoardIds,
      });

      // [JS] 응답 헤더(Authorization)에서 토큰을 추출
      const authHeader = response.headers['authorization'] || response.headers['Authorization'];
      const headerToken = authHeader?.startsWith('Bearer ') ? authHeader.substring(7) : authHeader;

      const findTokenInBody = (obj: any, targetKeys: string[]): string | null => {
        if (!obj || typeof obj !== 'object') return null;
        for (const key of Object.keys(obj)) {
          ...
          const nested = findTokenInBody(obj[key], targetKeys);
      ...
      const accessToken =
        headerToken ||
        findTokenInBody(userData, ['accessToken', 'token', 'access_token', 'jwt', 'data']);
      const refreshToken = findTokenInBody(userData, ['refreshToken', 'refresh_token', 'refresh']);

      console.log('--- [SOS] 최종 추출된 토큰 정보 ---', { accessToken, refreshToken });

      if (accessToken) {
        localStorage.setItem('accessToken', accessToken || '');
        Cookies.set('access_token', accessToken || '', { path: '/' });
      } else {
        console.error('--- [SOS] 응답 헤더와 바디 어디에서도 토큰을 찾지 못했습니다! ---', {
